    ...
    <div className="animate-fade-up">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative group flex-1 sm:flex-initial">
              ...
              <input 
                ...
                className="pl-10 pr-4 py-2.5 sm:py-2 bg-muted/50 border border-border/50 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 w-full sm:w-64 transition-all h-11 sm:h-10"
              />
            </div>
            <Button variant="outline" size="icon" onClick={fetchRepos} className="shrink-0 h-11 w-11 sm:h-10 sm:w-10">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-24 sm:py-32 gap-4">
          <Loader2 size={36} className="animate-spin text-primary sm:hidden" />
          <Loader2 size={40} className="animate-spin text-primary hidden sm:block" />
          <p className="text-xs sm:text-sm font-medium text-muted-foreground">Syncing repositories from GitHub...</p>
        </div>
      ) : (
          ...
          {filteredRepos.map((repo) => (
            <Card key={repo.id} className="card-hover flex flex-col group h-full">
              <CardHeader className="pb-3 flex-1 px-4 sm:px-6">
                ...
                <CardDescription className="text-xs sm:text-sm leading-relaxed line-clamp-3 mt-2 min-h-[3rem] sm:min-h-[4.5rem]">
      ...
      {!loading && filteredRepos.length === 0 && (
        <div className="text-center py-16 sm:py-24 bg-muted/20 rounded-2xl sm:rounded-3xl border border-dashed border-border">
          <p className="text-sm sm:text-base text-muted-foreground">No repositories found matching "{search}"</p>
          <Button variant="ghost" className="mt-4 h-10 sm:h-9" onClick={() => setSearch("")}>Clear Search</Button>
        </div>
      )}

      {!loading && (
        <div className="mt-14 sm:mt-20 p-6 sm:p-8 rounded-2xl sm:rounded-3xl bg-primary/5 border border-primary/10 text-center">
            <h3 className="text-lg sm:text-xl font-bold mb-2">Interested in my latest work?</h3>
